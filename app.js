...
let map,currentNestId=null,pendingNest=null,currentControl=0,markersVisible=true,speciesQuery='',selectedSpeciesCodes=new Set(),filterVisibility='all',filterYear='',editing=false,watchId=null,userPos=null,bdlEnabled=false,bdlLayer=null,bdlLabelLayer=null,bdlBoundaryLayer=null,bdlCompartmentBoundaryLayer=null,bdlSubareaBoundaryLayer=null,bdlBusy=false,bdlRequestSeq=0,bdlInfoPanel=null,bdlVectorLayer=null,bdlLabelsVisible=true,bdlVectorBusy=false,bdlVectorSeq=0,baseMapMode='imagery';
...
function initMap(){
 ...
 const bdlService='https://mapserver.bdl.lasy.gov.pl/arcgis/rest/services/WMS_BDL_mapa_drzewostanow/MapServer';
 bdlLayer=L.esri.dynamicMapLayer({url:bdlService,layers:[11],opacity:1,format:'png32',transparent:true,useCors:true}).setZIndex(200);
 bdlLabelLayer=L.esri.dynamicMapLayer({url:bdlService,layers:[3,5],opacity:1,format:'png32',transparent:true,useCors:true}).setZIndex(420);
 bdlBoundaryLayer=null; bdlCompartmentBoundaryLayer=null; bdlSubareaBoundaryLayer=null;
...
function updateBDLLabelToggle(){
 ...
 if(bdlLabelsVisible){
   ...
   if(bdlLabelLayer&&!map.hasLayer(bdlLabelLayer))bdlLabelLayer.addTo(map);
 }else{
   if(bdlLabelLayer&&map.hasLayer(bdlLabelLayer))map.removeLayer(bdlLabelLayer);
   if(bdlBoundaryLayer&&!map.hasLayer(bdlBoundaryLayer))bdlBoundaryLayer.addTo(map);
 }
}
function toggleBDLLabels(){bdlLabelsVisible=!bdlLabelsVisible;updateBDLLabelToggle();refreshBDLVectorOverlay()}
...
function toggleBDL(){
 ...
 if(bdlEnabled){
   ...
   bdlLayer.addTo(map);
   if(bdlLabelsVisible&&bdlLabelLayer)bdlLabelLayer.addTo(map);else if(bdlBoundaryLayer)bdlBoundaryLayer.addTo(map);
   setBDLBoundaryLayer();
...
function setBDLBoundaryLayer(){
 if(!bdlEnabled||!bdlLayer||!map)return;
 // Symbolika, granice i napisy są renderowane przez natywny ArcGIS Dynamic Map Layer.
 if(map.hasLayer(bdlLayer))bdlLayer.bringToFront();
}
async function refreshBDLVectorOverlay(){
 ...
 try{
   ...
   const comp=await queryLayer(3,'compartment_cd,a_year');
   const sub=z>=14?await queryLayer(5,'subarea_cd,a_year'):[];
   if(seq!==bdlVectorSeq)return;
   if(bdlVectorLayer)map.removeLayer(bdlVectorLayer);
   const all=[];
   if(z>=8){all.push(L.geoJSON(comp,{style:{color:'#aaa',weight:1,opacity:.9,fill:false}}));}
   if(z>=14&&sub.length)all.push(L.geoJSON(sub,{style:{color:'#555',weight:1,opacity:.9,fill:false}}));
   bdlVectorLayer=L.layerGroup(all);bdlVectorLayer.addTo(map);
   bdlBoundaryLayer=bdlVectorLayer;
 }catch(e){console.warn('BDL boundary overlay',e)}
}
...
function bdlPopupHtml(g,species){
 ...
 const speciesRows=[];
 const pushSpecies=(layer,code,part,age,extra)=>{
   const key=[layer,code,part,age,extra].map(v=>String(v??'').trim()).join('|');
   if(!code&&!layer)return;
   if(speciesRows.some(x=>x.key===key))return;
   speciesRows.push({key,html:`<div class="bdlSpeciesBlock"><div class="bdlRows">${row('Warstwa',layer)}${row('Gatunek',code)}${row('Udział',part)}${row('Wiek',age)}${extra?row('Pozostałe dane BDL',extra):''}</div></div>`});
 };
 const knownLayer=/^(DRZEW|PODSZ|PODS|NAL|PRZES|KRZ|II[Pp]|IIP|IIp|UPR|DOL|OL)$/i;
 const parseDesc=(raw)=>{
   if(raw==null||raw==='')return;
   let text=String(raw).trim();
   try{
     const j=JSON.parse(text); if(Array.isArray(j)){
       j.forEach(x=>pushSpecies(x.storey||x.layer||x.warstwa,x.species||x.species_cd||x.gatunek,x.part||x.part_cd||x.udzial,x.age||x.species_age||x.wiek,x.extra||''));
       if(speciesRows.length)return;
     }
   }catch{}
   const vals=text.split(';').map(v=>v.trim()).filter(Boolean);
   // Format BDL storey_species_desc jest sekwencją pól. Obsługujemy zarówno
   // wariant zaczynający się od dwóch pól technicznych, jak i wariant bez nich.
   const tryChunk=(offset)=>{
     let added=0;
     for(let i=offset;i<vals.length;i+=10){
       const r=vals.slice(i,i+10); if(r.length<4)break;
       let layer=r[2]||'', code=r[3]||'', part=r[4]||'', age=r[5]||'';
       if(!knownLayer.test(layer) && knownLayer.test(r[0]||'')){layer=r[0]||'';code=r[1]||'';part=r[2]||'';age=r[3]||''}
       if(!layer && !code)continue;
       const extra=r.slice(6).filter(Boolean).join(' · ');
       const before=speciesRows.length; pushSpecies(layer,code,part,age,extra); if(speciesRows.length>before)added++;
     }
     return added;
   };
   const before=speciesRows.length;
   tryChunk(0);
   if(speciesRows.length===before)tryChunk(2);
   // Awaryjnie rozpoznajemy fragmenty po nazwach warstw, aby nie zgubić danych
   // nawet gdy BDL zmieni układ pól w opisie.
   if(speciesRows.length===before){
     for(let i=0;i<vals.length;i++)if(knownLayer.test(vals[i])){
       pushSpecies(vals[i],vals[i+1]||'',vals[i+2]||'',vals[i+3]||'',vals.slice(i+4,i+10).filter(Boolean).join(' · '));
     }
   }
 };
 // Najważniejsze: pełny opis z Mobile/Opis_taksacyjny_mBDL zawiera wszystkie
 // gatunki ze wszystkich warstw. Nie wolno zastępować go jednym rekordem dominującym.
 parseDesc(a.storey_species_desc);
 // Jeśli pełny opis nie był dostępny, wykorzystaj rekordy z warstwy mapowej.
 const renderSpeciesFeatures=()=>{
   (species||[]).forEach(x=>{
     ...
     pushSpecies(p.storey_cd,p.species_cd,p.part_cd,p.species_age,details.map(([k,v])=>`${k}: ${v}`).join(' · '));
   });
 };
 if(!speciesRows.length)renderSpeciesFeatures();
 const speciesHtml=speciesRows.length?speciesRows.map(x=>x.html).join(''):'<div class="muted">Brak szczegółowych danych o gatunkach.</div>';
...
function renderMarkers(){if(!map)return;map.eachLayer(l=>{if(l instanceof L.Marker)map.removeLayer(l)});if(!markersVisible)return;nests.filter(n=>n.lat&&n.lon&&!n.hidden&&nestMatches(n)).forEach(n=>{const m=L.marker([n.lat,n.lon]).addTo(map);let tip=n.label||'KOD';m.bindTooltip(esc(tip),{permanent:true,direction:'top',className:'marker-label'});m.on('click',()=>openNest(n.id,false));})}
...
function setEditable(v){const n=getCurrentNest();const c=n?.controls?.[currentControl];const saved=!!c&& (c.saved===true||hasLegacySavedControl(n,currentControl,c));editing=saved?!(!v):true; if(saved && !v) editing=false; if(!saved) editing=true; const canEdit=editing;['birdInput','criterion','obsPickerBtn','treeInput','count','chicks','notes'].forEach(id=>{const e=$(id);if(e)e.disabled=!canEdit});const d=$('date'),t=$('time');if(d)d.readOnly=true;if(t)t.readOnly=true;const saveBtn=$('saveControl');if(saveBtn)saveBtn.hidden=!canEdit;const eb=$('editNestBtn');if(eb){eb.hidden=canEdit||!saved;eb.textContent='✏️ Edytuj'}}
function getCurrentNest(){return pendingNest&&pendingNest.id===currentNestId?pendingNest:nests.find(x=>x.id===currentNestId)}
function saveCurrent(){
 const n=getCurrentNest(); if(!n)return;
 const c=n.controls[currentControl]||blankControl();
 if(currentControl===0&&!n.bird){
   const q=$('birdInput')?.value.trim().toLocaleLowerCase('pl-PL'),b=BIRDS.find(x=>x.name.toLocaleLowerCase('pl-PL')===q||x.code.toLocaleLowerCase('pl-PL')===q);
 ...
 c.count=$('count').value;c.chicks=$('chicks').value;c.criterion=$('criterion').value;c.observations=[...document.querySelectorAll('[data-obs]:checked')].map(x=>OBS[+x.dataset.obs]);c.tree=$('treeInput').value.trim();
 const t=TREES.find(x=>x.name.toLocaleLowerCase('pl-PL')===c.tree.toLocaleLowerCase('pl-PL')||x.code.toLocaleLowerCase('pl-PL')===c.tree.toLocaleLowerCase('pl-PL'));c.treeCode=t?t.code:($('treeInput').dataset.code||'');
 const x=now();if(!c.saved){c.date=x.date;c.time=x.time}c.notes=$('notes').value;c.saved=true;n.controls[currentControl]=c;n.draft=false;n.seasons=n.seasons||{};const year=c.date.slice(0,4);n.seasons[year]=n.seasons[year]||[];n.seasons[year][currentControl]=JSON.parse(JSON.stringify(c));
 // Dopiero tutaj punkt trafia do localStorage i staje się gniazdem na mapie.
 if(!nests.some(x=>x.id===n.id))nests.push(n);
 pendingNest=null;save();$('nestCode').textContent=n.label;editing=false;closeNest();
}
function openNest(id){const n=(pendingNest&&pendingNest.id===id)?pendingNest:nests.find(x=>x.id===id);if(!n)return;currentNestId=id;currentControl=0;editing=!!n.draft;$('nestCard').hidden=false;$('nestCode').textContent=n.label||'nie nadano';$('birdFixed').innerHTML=n.bird?`<strong>Gatunek: ${esc(n.bird)} (${esc(n.birdCode)})</strong>`:'Gatunek: jeszcze nie wybrano';$('nestVisibilityBtn').textContent='Ukryj gniazdo';$('nestVisibilityBtn').onclick=()=>{n.hidden=true;save();renderMarkers();closeNest()};$('editNestBtn').onclick=()=>{editing=true;renderControl()};$('deleteNestBtn').onclick=()=>{if(confirm('Usunąć to gniazdo wraz ze wszystkimi kontrolami?')){nests=nests.filter(x=>x.id!==id);save();closeNest()}};$('historyBtn').onclick=()=>renderHistory(n);renderTabs();renderControl();requestAnimationFrame(()=>{const card=$('nestCard');card.scrollIntoView({behavior:'smooth',block:'start'});const title=$('nestTitle');if(title)title.focus({preventScroll:true})})}
...
function renderTabs(){document.querySelectorAll('.tab').forEach((b,i)=>{b.classList.toggle('active',i===currentControl);b.onclick=()=>{currentControl=i;const n=getCurrentNest();const c=n?.controls?.[i]||blankControl();editing=!!n?.draft || !c.saved;renderTabs();renderControl()}})}
function cancelCurrentNest(){const n=getCurrentNest();if(!n)return;if(n.draft&&pendingNest?.id===n.id){pendingNest=null;currentNestId=null;$('nestCard').hidden=true;$('mapMode').hidden=false;renderMarkers();return}editing=false;renderControl()}
function closeNest(){
 const n=getCurrentNest();
 // Niezapisany formularz jest wyłącznie obiektem roboczym i nigdy nie jest zapisywany.
 if(n?.draft && pendingNest?.id===n.id)pendingNest=null;
 currentNestId=null;$('nestCard').hidden=true;$('mapMode').hidden=false;$('historyPanel').hidden=true;renderMarkers();map.invalidateSize();window.scrollTo({top:0,behavior:'smooth'});
}
function registerNest(){
 const c=map.getCenter(),id=crypto.randomUUID?crypto.randomUUID():String(Date.now());
 pendingNest={id,lat:Number(c.lat.toFixed(6)),lon:Number(c.lng.toFixed(6)),bird:'',birdCode:'',label:'',number:'',controls:[blankControl(),blankControl(),blankControl(),blankControl()],seasons:{},draft:true};
 currentNestId=id;$('mapMode').hidden=true;$('nestCard').hidden=false;openNest(id);
 requestAnimationFrame(()=>{const card=$('nestCard');if(card){card.hidden=false;card.scrollIntoView({behavior:'smooth',block:'start'})}});
}
...
function exportGPX(){
 ...
 const w=list.map(n=>{const nm=n.label||'KOD',species=n.bird||'';const name=species?`${nm} — ${species}`:nm;return `<wpt lat="${Number(n.lat).toFixed(6)}" lon="${Number(n.lon).toFixed(6)}"><name>${escXml(name)}</name></wpt>`}).join('');

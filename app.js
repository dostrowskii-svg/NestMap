...
async function fetchBDLExportData(n){
 const empty={bdlAddress:'',bdlCompartment:'',bdlAge:'',bdlSpecies:'',bdlTSL:''};
 if(!n||!Number.isFinite(Number(n.lat))||!Number.isFinite(Number(n.lon)))return empty;
 try{
  const makeParams=(lat,lon,inSR)=>new URLSearchParams({where:'1=1',geometry:`${lon},${lat}`,geometryType:'esriGeometryPoint',inSR:String(inSR),spatialRel:'esriSpatialRelIntersects',outFields:'*',returnGeometry:'false',resultRecordCount:'1',f:'json'});
  let j=null;
  for(const q of [makeParams(Number(n.lat),Number(n.lon),4326),makeParams(...webMercator(Number(n.lat),Number(n.lon)).reverse(),3857)]){
   const r=await fetch(`${BDL_VECTOR_URL}?${q}`,{cache:'no-store'});if(!r.ok)continue;
   const x=await r.json();if(x.features?.length){j=x;break}
  }
  const a=j?.features?.[0]?.attributes||{}; if(!Object.keys(a).length)return empty;
  ...
  return {
   ...
   bdlAge:a.species_age??'',
   bdlSpecies:a.species_cd_d||a.species_cd||'',
   bdlTSL:a.site_type_cd||''
  };
 }catch(e){console.warn('BDL export:',e);return empty}
}
async function exportCSV(){
 ...
 await shareOrDownload(csv,'NestMap.csv','text/csv;charset=utf-8')
}
...
async function exportKMZ(){
 ...
 const k=await Promise.all(list.map(async n=>{
  ...
  const bdl=`<h3>BDL — dane do eksportu</h3><table><tr><td>Adres wydzielenia</td><td>${esc(kmzValue(b.bdlAddress))}</td></tr><tr><td>Numer wydzielenia</td><td>${esc(kmzValue(b.bdlCompartment))}</td></tr><tr><td>Wiek drzewostanu</td><td>${esc(kmzValue(b.bdlAge))}</td></tr><tr><td>Gatunek dominujący</td><td>${esc(kmzValue(b.bdlSpecies))}</td></tr><tr><td>TSL</td><td>${esc(kmzValue(b.bdlTSL))}</td></tr></table>`;
 ...
 $('mapStatus').textContent=''; await shareOrDownload(zip,'NestMap.kmz','application/vnd.google-earth.kmz');
}
function exportGPX(){
 const list=getFilteredNests().filter(n=>!window.__nmExportBounds||window.__nmExportBounds.contains([n.lat,n.lon]));
 if(!list.length)return alert('Brak gniazd spełniających wybrane filtry.');
 const escXml=v=>String(v??'').replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/"/g,'&quot;').replace(/'/g,'&apos;');
 const w=list.map(n=>{const nm=n.label||'gniazdo',species=n.bird||'';const name=species?`${nm} — ${species}`:nm;const controls=(n.controls||[]).map((c,i)=>`Kontrola ${i+1}: ${c?.date||''} ${c?.time||''}; kryterium: ${(c?.criterion||'')}; obserwacje: ${(c?.observations||[]).join(', ')}; liczebność: ${c?.count||''}; pisklęta: ${c?.chicks||''}; drzewo: ${c?.tree||''}; uwagi: ${c?.notes||''}`).join('\n');return `<wpt lat="${Number(n.lat).toFixed(6)}" lon="${Number(n.lon).toFixed(6)}"><name>${escXml(name)}</name><desc>${escXml(`KOD: ${n.label||''}\nGatunek: ${species}\nKod gatunku: ${n.birdCode||''}\n${controls}`)}</desc></wpt>`}).join('');
 ...
 shareOrDownload(gpx,'NestMap.gpx','application/gpx+xml');
}
async function shareOrDownload(data,name,type){
 const blob=data instanceof Blob?data:new Blob([data],{type});
 const file=new File([blob],name,{type});
 try{
  if(navigator.share&&(!navigator.canShare||navigator.canShare({files:[file]}))){await navigator.share({files:[file],title:name});return true}
 }catch(e){if(e?.name==='AbortError')return true;console.warn('Udostępnianie pliku:',e)}
 downloadBlob(blob,name,type);return false;
}
function downloadBlob(data,name,type){const blob=data instanceof Blob?data:new Blob([data],{type});const a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=name;a.rel='noopener';document.body.appendChild(a);a.click();setTimeout(()=>{URL.revokeObjectURL(a.href);a.remove()},10000)}
...
function dbfNum(v,len,dec=0){let s=(v==null||v==='')?'':Number(v).toFixed(dec);if(s.length>len)s=s.slice(-len);return dbfText(s,len)}
function buildShapefile(list,bdlDataMap={}){
 ...
 for(let i=0;i<n;i++){
  ...
  for(let j=0;j<fields.length;j++){const f=fields[j];dbf.set(f[1]==='N'?dbfNum(vals[j],f[2],f[3]):dbfText(vals[j],f[2]),q);q+=f[2]}
 }
 dbf[dbf.length-1]=26;
...
async function exportSHP(){
 ...
 $('mapStatus').textContent='';await shareOrDownload(zipStore(files),'NestMap_SHP.zip','application/zip')
}
function exportGPX(){
 const list=getFilteredNests().filter(n=>!window.__nmExportBounds||window.__nmExportBounds.contains([n.lat,n.lon]));
 if(!list.length)return alert('Brak gniazd spełniających wybrane filtry.');
...
function applyAreaExport(){window.__nmExportBounds=map.getBounds();alert('Ustawiono eksport dla obecnie widocznego fragmentu mapy. CSV, KMZ, GPX i SHP będą ograniczone do tego obszaru.')}

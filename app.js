...
function openNest(id){const n=nests.find(x=>x.id===id);if(!n)return;currentNestId=id;currentControl=0;editing=!!n.draft;$('nestCard').hidden=false;$('nestCode').textContent=n.label||'nie nadano';$('birdFixed').innerHTML=n.bird?`<strong>Gatunek: ${esc(n.bird)} (${esc(n.birdCode)})</strong>`:'Gatunek: jeszcze nie wybrano';$('nestVisibilityBtn').textContent='Ukryj gniazdo';$('nestVisibilityBtn').onclick=()=>{n.hidden=true;save();renderMarkers();closeNest()};$('editNestBtn').onclick=()=>{editing=true;renderControl()};$('deleteNestBtn').onclick=()=>{if(confirm('Usunąć to gniazdo wraz ze wszystkimi kontrolami?')){nests=nests.filter(x=>x.id!==id);save();closeNest()}};$('historyBtn').onclick=()=>renderHistory(n);renderTabs();renderControl();requestAnimationFrame(()=>{const card=$('nestCard');if(card){const top=Math.max(0,card.getBoundingClientRect().top+window.scrollY-8);window.scrollTo({left:0,top,behavior:'smooth'});document.documentElement.scrollLeft=0;document.body.scrollLeft=0}const title=$('nestTitle');if(title)title.focus({preventScroll:true})})}
...
function closeNest(){const n=currentNestId?nests.find(x=>x.id===currentNestId):null;if(n?.draft){nests=nests.filter(x=>x.id!==currentNestId);save()}currentNestId=null;$('nestCard').hidden=true;$('mapMode').hidden=false;$('historyPanel').hidden=true;renderMarkers();map.invalidateSize();window.scrollTo({left:0,top:0,behavior:'smooth'});document.documentElement.scrollLeft=0;document.body.scrollLeft=0}

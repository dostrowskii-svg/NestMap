...
function preloadLeafletMarkerAssets(){try{['marker-icon.png','marker-icon-2x.png','marker-shadow.png'].forEach(n=>{const i=new Image();i.src='https://unpkg.com/leaflet@1.9.4/dist/images/'+n})}catch(e){}}
function initMap(){
 preloadLeafletMarkerAssets();
...
c.count=$('count').value;c.chicks=$('chicks').value;c.criterion=$('criterion').value;c.observations=[...document.querySelectorAll('[data-obs]:checked')].map(x=>OBS[+x.dataset.obs]);c.tree=$('treeInput').value.trim();const t=TREES.find(x=>x.name.toLocaleLowerCase('pl-PL')===c.tree.toLocaleLowerCase('pl-PL')||x.code.toLocaleLowerCase('pl-PL')===c.tree.toLocaleLowerCase('pl-PL'));c.treeCode=t?t.code:($('treeInput').dataset.code||'');const x=now();if(!c.saved){c.date=x.date;c.time=x.time}c.notes=$('notes').value;c.saved=true;n.controls[currentControl]=c;n.draft=false;n.seasons=n.seasons||{};const year=c.date.slice(0,4);n.seasons[year]=n.seasons[year]||[];n.seasons[year][currentControl]=JSON.parse(JSON.stringify(c));save();$('nestCode').textContent=n.label;editing=false;closeNest();renderMarkers()}
function scrollToNestForm(){const card=$('nestCard');if(!card)return;const top=Math.max(0,card.getBoundingClientRect().top+window.pageYOffset-8);try{window.scrollTo({top,left:0,behavior:'auto'})}catch(e){window.scrollTo(0,top)}document.documentElement.scrollLeft=0;document.body.scrollLeft=0;setTimeout(()=>{try{window.scrollTo({top,left:0,behavior:'auto'})}catch(e){window.scrollTo(0,top)}document.documentElement.scrollLeft=0;document.body.scrollLeft=0},40)}
function openNest(id){const n=nests.find(x=>x.id===id);if(!n)return;currentNestId=id;currentControl=0;editing=!!n.draft;$('nestCard').hidden=false;$('nestCode').textContent=n.label||'nie nadano';$('birdFixed').innerHTML=n.bird?`<strong>Gatunek: ${esc(n.bird)} (${esc(n.birdCode)})</strong>`:'Gatunek: jeszcze nie wybrano';$('nestVisibilityBtn').textContent='Ukryj gniazdo';$('nestVisibilityBtn').onclick=()=>{n.hidden=true;save();renderMarkers();closeNest()};$('editNestBtn').onclick=()=>{editing=true;renderControl()};$('deleteNestBtn').onclick=()=>{if(confirm('Usunąć to gniazdo wraz ze wszystkimi kontrolami?')){nests=nests.filter(x=>x.id!==id);save();closeNest()}};$('historyBtn').onclick=()=>renderHistory(n);renderTabs();renderControl();requestAnimationFrame(()=>{scrollToNestForm();const title=$('nestTitle');if(title)title.focus({preventScroll:true})})}
...
function closeNest(){const n=currentNestId?nests.find(x=>x.id===currentNestId):null;if(n?.draft){nests=nests.filter(x=>x.id!==currentNestId);save()}currentNestId=null;$('nestCard').hidden=true;$('mapMode').hidden=false;$('historyPanel').hidden=true;map.invalidateSize();window.scrollTo({left:0,top:0,behavior:'auto'});document.documentElement.scrollLeft=0;document.body.scrollLeft=0}
